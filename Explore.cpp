...
protected:
    ...
    void MoveMouseRandomly()
    {
        ...
        std::uniform_int_distribution<int> dx(r.left + 20, r.right - 20);
        std::uniform_int_distribution<int> dy(r.top + 20, r.bottom - 20);

        POINT start{};
        GetCursorPos(&start);
        POINT end{ dx(gen), dy(gen) };

        for (int i = 1; i <= 18; ++i)
        {
            SetCursorPos(start.x + (end.x - start.x) * i / 18,
    ...
    {
        static std::mt19937 gen{ std::random_device{}() };
        std::uniform_int_distribution<UINT> delay(1800, 5200);
        SetTimer(ID_TIMER_MOVE, delay(gen), nullptr);
    }
